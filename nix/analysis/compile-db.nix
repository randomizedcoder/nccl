# Generate compile_commands.json for the NCCL repo.
#
# Strategy:
...
let
  ...
  syntheticDbScript = pkgs.writeText "synthetic-compile-db.py" ''
    ...
    # Directories to skip
    skip_dirs = {'build', 'build-*', '.git', 'pkg', 'makefiles', '__pycache__'}

    # NCCL-specific include paths
    include_flags = ' '.join([
        f'-I{source_dir}/src/include',
        f'-I{source_dir}/src/include/plugin',
        f'-I{source_dir}/src/device',
        f'-I{source_dir}/src',
    ])

    # NCCL-specific defines
    defines = ' '.join([
        '-DNCCL_MAJOR=2',
        '-DNCCL_MINOR=30',
        '-DNCCL_PATCH=4',
  ...
  compileDb = pkgs.runCommand "compile-db-nccl" {
    nativeBuildInputs = with pkgs; [ cmake python3 gnumake gcc ] ++ cudaDeps;
  } ''
    mkdir -p $out

    echo "=== Generating compile_commands.json for NCCL ==="
    ...
    if cmake -B source/build \
      ...
      if [ -f source/build/compile_commands.json ]; then
        ...
        # Replace /build/source with the actual source store path, and point
        # directory entries to the source root (build dirs don't persist).
...
" $out/compile_commands.json
        ...
        echo "cmake configured but no compile_commands.json found, using synthetic"
