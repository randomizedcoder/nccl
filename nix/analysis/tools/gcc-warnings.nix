...
# that the default build misses. Since NCCL requires CUDA for a full build,
...
let
  ...
  runner = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      # NCCL requires CUDA for full cmake build, so compile individual
      ...
      while IFS= read -r file; do
        ...
        g++ -fsyntax-only \
          ...
          -I"$source_dir/src/include/plugin" \
          -I"$source_dir/src/device" \
          -I"$source_dir/src" \
          "$file" 2>&1 | \
